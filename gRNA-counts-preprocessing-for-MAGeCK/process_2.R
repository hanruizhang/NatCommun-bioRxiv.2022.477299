...
input_2 <- "input/Gene_sgRNA_S1S2_ANOTATED.txt"
input_3 <- "input/Screen1+3_All-in-one_Read-Count_Calculation.txt"
...
# Remove duplicate sgRNAs from dat_1 and dat_3
dat_1.dedup <- dat_1[! duplicated(dat_1$sgRNA), ]
dat_3.dedup <- dat_3[! duplicated(dat_3$sgRNA), ]

# Select columns from dat_2 and dat_3
dat_2 <- dat_2[, c(1, 3:ncol(dat_2))]
dat_3.dedup <- dat_3.dedup[, c(1, grep("S3$", colnames(dat_3.dedup)))]

# Merge
merge_12 <- merge(dat_1.dedup, dat_2, by = 1)
